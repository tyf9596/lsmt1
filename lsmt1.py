# -*- coding: utf-8 -*-
"""
Created on Thu Jul  5 10:35:33 2018
...
#input1=pre_process(mnist.train.images,55000,784)
#input2=pre_process(mnist.validation.images,5000,784)
#test=pre_process(mnist.test.images,5000,784)

#input1=straight_cs(mnist.train.images,obmatrix,55000,compression_nodenum)
#print(input1.shape)
#input2=straight_cs(mnist.validation.images,obmatrix,5000,compression_nodenum)
test=straight_cs(mnist.test.images,obmatrix,10000,compression_nodenum)
...
_X = tf.placeholder(tf.float32, [None, compression_nodenum])
...
for i in range(15000):
    ...
    if (i+1)%200 == 0:
        ...
        train_accuracy = sess.run(accuracy, feed_dict={
            _X:np.matmul(batch[0],obmatrix), y: batch[1], keep_prob: 1.0, batch_size: _batch_size})
        # 已经迭代完成的 epoch 数: mnist.train.epochs_completed
        print ("Iter%d, step %d, training accuracy %g" % ( mnist.train.epochs_completed, (i+1), train_accuracy))
    sess.run(train_op, feed_dict={_X: np.matmul(batch[0],obmatrix), y: batch[1], keep_prob: 0.5, batch_size: _batch_size})

# 计算测试数据的准确率
print(test.shape)
print( mnist.test.images.shape)
print ("test accuracy %g"% sess.run(accuracy, feed_dict={
    _X: test, y: mnist.test.labels, keep_prob: 1.0, batch_size:mnist.test.images.shape[0]}))
